...
//					double f2 = 168 * E * Math.pow(RM, 6) * (-4 * Math.pow(distance, 6) + 13 * Math.pow(RM, 6)) / Math.pow(distance, 15);
//					Pair d2 = new Pair(f2 * dx / distance, f2 * dy / distance);
//					double f3 = -504 * E * Math.pow(RM, 6) * (-12 * Math.pow(distance, 6) + 65 * Math.pow(RM, 6)) / Math.pow(distance, 16);
