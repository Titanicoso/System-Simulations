...
public class GearPredictorCorrector implements Integrator {
    ...
    public Particle evolve(final Particle particle, final double dt,
        ...
        final int alphaIndex = forces.stream().anyMatch(force -> force.isVelocityDependant()) ? 1 : 0;
        ...
        return new Particle(particle.getId(), correctedR.getX(), correctedR.getY(), correctedR1.getX(),
                correctedR1.getY(), mass, particle.getRadius());
    }
}
